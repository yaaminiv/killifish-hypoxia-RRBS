...
NBH20vOCavgMeth <- NBH20vOCpositions %>%
  ...
  slice(., 1, 3:10, 2) #Group by DMR. Remove chromosome and position columns. Average methylation level across all CpG positions in each DMR for each sample. Reorganize manually with slice.
...
NBH5vOCavgMeth <- NBH5vOCpositions %>%
  ...
  dplyr::select(., -c(chr, pos)) %>%
  summarize_all(funs(sum(., na.rm = TRUE))) %>%
  slice(., 1, 12, 23, 34, 45, 56:59, 2:11, 13:22, 24:33, 35:44, 46:55) #Group by DMR. Remove chromosome and position columns. Average methylation level across all CpG positions in each DMR for each sample. Reorganize manually with slice
